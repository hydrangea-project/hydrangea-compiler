...
cabal run hydrangea-compiler -- \
  --all-top-level-procs \
  --no-solver-check \
  --output-c="$OUT_DIR/wave.c" \
  --output-h="$OUT_DIR/wave.h" \
  --export-kernel=wave_step \
  "$DEMO_DIR/wave.hyd"
...
echo ""
echo "=== Done ==="
echo "Run with:  $OUT_DIR/wave_sdl"
echo ""
echo "Controls:"
echo "  Left mouse click/drag  Inject a displacement pulse"
