...
# ---------------------------------------------------------------------------
# Stage 1: Generate Metal export (640x480)
# ---------------------------------------------------------------------------
echo "=== Generating Metal export kernel (fancy 640x480) ==="
cabal run hydrangea-compiler -- \
  --all-top-level-procs --no-solver-check \
  --metal \
  --export-metal-kernel=voxel_render \
  --output-metal="$OUT_DIR/voxel_fancy.metal" \
  --output-c="$OUT_DIR/voxel_fancy_metal.m" \
  --output-h="$OUT_DIR/voxel_fancy_metal.h" \
...
xcrun -sdk macosx metal -c -o "$OUT_DIR/voxel_fancy.air" "$OUT_DIR/voxel_fancy.metal"
xcrun -sdk macosx metallib -o "$OUT_DIR/voxel_fancy.metallib" "$OUT_DIR/voxel_fancy.air"
...
clang -O2 -fobjc-arc \
  ...
  -c "$OUT_DIR/voxel_fancy_metal.m" \
  -o "$OUT_DIR/voxel_fancy_metal.o"
...
clang++ -O2 -std=c++17 \
  ...
  '-DMETALLIB_NAME="voxel_fancy.metallib"' \
  -I"$OUT_DIR" -Iruntime -Ithird_party/simde \
  -include "$OUT_DIR/voxel_fancy_metal.h" \
  -c "$DEMO_DIR/main.cpp" \
  -o "$OUT_DIR/main_metal.o"

echo "=== Linking ==="
clang++ \
  "$OUT_DIR/voxel_fancy_metal.o" \
  "$OUT_DIR/main_metal.o" \
  $SDL2_LIBS \
  -framework Metal -framework Foundation \
  -o "$OUT_DIR/voxel_fancy_sdl_metal"

echo ""
echo "=== Done ==="
echo "Run with:  $OUT_DIR/voxel_fancy_sdl_metal"
echo ""
echo "Controls:"
