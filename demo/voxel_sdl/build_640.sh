...
# ---------------------------------------------------------------------------
# Stage 1: Compile Hydrangea → C (640×480)
# ---------------------------------------------------------------------------
echo "=== Generating exported Hydrangea kernel (640×480) ==="
cabal run hydrangea-compiler -- \
  --all-top-level-procs \
  --no-solver-check \
  --output-c="$OUT_DIR/voxel_live_640.c" \
  --output-h="$OUT_DIR/voxel_live_640.h" \
...
"$CC_BIN" -O2 -std=c99 -fopenmp \
  ...
  -c "$OUT_DIR/voxel_live_640.c" \
  -o "$OUT_DIR/voxel_live_640.o"
...
"$CXX_BIN" -O2 -std=c++17 -fopenmp \
  ...
  -I"$OUT_DIR" -Iruntime -Ithird_party/simde \
  -include "$OUT_DIR/voxel_live_640.h" \
  -c "$DEMO_DIR/main.cpp" \
  -o "$OUT_DIR/main_640.o"

echo "=== Linking ==="
"$CXX_BIN" -fopenmp \
  "$OUT_DIR/voxel_live_640.o" \
  "$OUT_DIR/main_640.o" \
  $SDL2_LIBS \
  -o "$OUT_DIR/voxel_sdl_640"

echo ""
echo "=== Done ==="
echo "Run with:  $OUT_DIR/voxel_sdl_640"
echo ""
echo "Controls:"
